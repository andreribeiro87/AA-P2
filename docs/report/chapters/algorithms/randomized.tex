...
efficiency~\cite{motwani1995randomized}. We implement five randomized
approaches for MWC.

\subsubsection{Random Construction}
\label{sec:random-construction}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State $candidates \gets candidates \setminus \{u\}$
        \EndWhile
        \If{$W(C) > W^*$}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \For{$s = 1$ to $S$}
        \State $v \gets$ \textsc{RandomChoice}($V$)
        \State $C \gets \{v\}$, $candidates \gets V \setminus \{v\}$
        \While{$candidates \neq \emptyset$}
        \State $compatible \gets \{(u, w(u)) : u \in candidates, \forall c \in C, (u,c) \in E\}$
        \If{$compatible = \emptyset$}
        \State \textbf{break}
        \EndIf
        \If{\textsc{Random}$(0,1) < \rho$}
        \State $u \gets$ \textsc{RandomChoice}($compatible$)
        \Else
...
\subsubsection{Iterative Random Search}
\label{sec:iterative-random-search}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State $available \gets available \setminus \{v\}$
        \EndWhile
        \If{$W(C) > W^*$}
        \State $C^* \gets C$, $W^* \gets W(C)$
        \EndIf
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State $C^* \gets \emptyset$, $W^* \gets 0$
        \For{$a = 1$ to $A$}
        \If{strategy = ``random\_walk''}
        \State $C \gets$ \textsc{RandomWalkClique}($G$)
        \Else
        \State $C \gets$ \textsc{IterativeConstruction}($G$)
        \EndIf
        \If{\textsc{VerifyClique}($C$)} \Comment{Always verify}
        \If{$W(C) > W^*$}
        \State $C^* \gets C$, $W^* \gets W(C)$
        \EndIf
        \EndIf
        \EndFor
        \If{$C^* = \emptyset$} \Comment{Fallback guarantee}
        \State $C^* \gets \{\arg\max_{v \in V} w(v)\}$
        \EndIf
        \State \Return $C^*$
    \end{algorithmic}
\end{algorithm}
...
Table~\ref{tab:randomized-comparison} summarizes the key characteristics of our
